...
export async function getURL(alias: string): Promise<string | null> {
    const results = await prisma.links.findFirst({
        where: {
            aliases: {
                has: alias
            }
...
export async function create_redirect(url: string, aliases: string[]) {
    try {
      ...
      if (existingLinks.length > 0 || existingURL) {
        console.log(chalk.red('Some link/aliases already exist:', existingLinks, existingURL));
...
export async function update_redirect(url: string, aliases: string[]) {
  try {
    ...
    if (!existingURL) {
      console.log(chalk.red(`Cannot find existing URL from ${url}, logs:\n`, existingURL));
    ...
    console.log(chalk.green(`Updated: ${url} with aliases ${aliases.join(', ')}.`))
    return newLink;
  } catch (error) {
...
export async function delete_redirect(aliases: string[]) {
  try {
    ...
    // If any aliases or the URL already exist, handle the conflict
    if (!existingLinks) {
      console.log(chalk.red(`Cannot find existing URLs from ${aliases.join(', ')}, logs:\n`, existingLinks));
